...
import { useParams, history, useOutletContext, History } from "umi";

export default function PostPage(props?: any) {
	console.log(useOutletContext());
	let { methods } = useOutletContext<{
		methods: {
			setButtons: ([]: Iterable<{
				text: string;
				click: (h: History) => void;
			}>) => void;
		};
	}>();
	...
	useEffect(() => {
		...
		methods.setButtons([
			{
				text: "编辑",
				click: (h) => h.push(`/posts/${params.postId}/edit`),
			},
		]);

		return () => methods.setButtons([]);
	}, []);
						...
						<div className="w-full absolute lg:bottom-24 bottom-12 container lg:px-64 px-8">
							...
							<div className="flex flex-row mt-8 align-bottom">
								...
								<a
									...
									onClick={() => {
									outletContext.methods.setButtons([
										{
										text: "编辑",
										click: () =>
											history.push(`/posts/${params.postId}/edit`),
										},
									]);
									}}
								>
